...
public function atualizar_informacoes_pessoais(Request $r): void
{
    ...
    new Alert("Não foi possivel atualizar os dados", "erro", "Erro ao atualizar os dados");
    Http::redirecionar("/usuario/meu_perfil");
    ...
    public function atualizar_usuario(Request $r): void
    {
        ...
        if (Testos::desincriptar($usuarioLogado["senha"]) !==  $r->post("senhaC")){
            new Alert("Senha incorreta", "erro", "Senha incorrecta");
            Http::redirecionar("/usuario/meu_perfil");
            return;
        }
        ...
        if ($usuarioLogado["email"] != $r->post("u_email")){
            ...
            ->first();
            if ($usuario != null){
                new Alert("Não foi possivel atualizar os dados.", "erro","Email já registrado.");
                Http::redirecionar("/usuario/meu_perfil");
                return;
            }
        ...
        if ($usuarioLogado["contacto"] != $r->post("u_contacto")){
            ...
            ->first();
            if ($usuario != null){
                new Alert("Não foi possivel atualizar os dados.", "erro","Contacto já registrado.");
                Http::redirecionar("/usuario/meu_perfil");
                return;
            }
    ...
    public function atualizar_senha(Request $r): void
    {
        ...
        if ($r->post("s_nova_senha_c") !==  $r->post("s_nova_senha")){
            new Alert(" Nova senha não é igual a confirmação da nova senha", "erro", "Senha não correspondem");
            Http::redirecionar("/usuario/meu_perfil");
            return;
        }
        $usuario = Usuarios::where(['id' => $usuarioLogado["id"]])
