...
class Biblioteca extends Controller
{
    ...
    public function curso_por_livros(int $id = 0, Request $r)
    {
        $this->verifica();
        $cursos = Cursos::all();
        $livros = Livros::where("cursos_id", "=", $r->get("q"))->get();
        $curso = null;
        if (!isset($livros)) {
    ...
    public function salvar_livro(Request $req)
    {
        ...
        if ($ficheiro != null) {
            if (!Ficheiros::eDocumentoValido($ficheiro)) {
                new Alert("Envie um ficheiro valido.", "danger", "Documento invalido");
        ...
        if ($capa != null) {
            if (!Ficheiros::eImagemValida($capa)) {
                new Alert("A capa do livro não é valida.", "danger", "Capa invalida.");
                Http::redirecionar("/biblioteca/adicionar_livro");
                return;
            }
        ...
        try {
            ...
            $livro->save();
            new Alert("O livro " . $titulo . " foi salvo.", "success");
            Http::redirecionar("/biblioteca/adicionar_livro");
        } catch (FileException $e) {
            new Alert("Motivado por: <b>" . $e->getMessage(), "danger", "Erro critico");
            Http::redirecionar("/biblioteca/adicionar_livro");
            return;
        }
    }
    ...
    public function eliminar_livro(int $id)
    {
        ...
        if ($livro != null) {
            try {
                ...
                new Alert("Não foi possivel eliminar o livro pelo movito:" . $e->getMessage(), "danger", "Erro critico");
                Http::redirecionar("/biblioteca/todos_livros");
                return;
            }
        }
        new Alert("Não é possivel eliminar um livro que não existe!", "danger", "Erro ao eliminar o livro.");
        Http::redirecionar("/biblioteca/todos_livros");
    }
    #Gestão de cursos
